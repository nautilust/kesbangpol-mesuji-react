...
export default function KategoriBerita() {
  ...
  const [beritaData, setBeritaData] = useState([]),
    ...
    [kategoriTitle, setKategoriTitle] = useState(`${idKategori}`),
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const beritaResult = await axios.get(
          `${API_URL}/news?instansi_id=${INS_ID}&category_id=${idKategori}${titleData}&per-page=8`,
          { cancelToken: source.token, timeout: 60000 }
        );

        if (beritaResult.data.items.length === 0) {
          setStatus("data kosong");
        } else {
          setBeritaData(beritaResult.data);
          setKategoriTitle(beritaResult.data.items[0].kategori);
          setStatus("selesai");
        }
  ...
  }, [halaman, statusSearch, searchTitle, idKategori]);
  ...
  function BeritaSection() {
    switch (status) {
      ...
      case "selesai":
        return beritaData.items.map((item, i) => (
  ...
  // TODO: ketika sudah ada api berita populer sebaiknya fungsi ini dibuat terpisah
      ...
      <div className="container">
        ...
        <div className="row">
            ...
            {status === "selesai" && statusSearch === true && (
              <div className="py-1 d-flex">
                <h5 className="me-auto">
                  ...
                  {beritaData._meta.totalCount} data)
                </h5>
                <button
              ...
              {status === "memuat" ? (
                ...
                status === "selesai" && (
                  <Pagination
                    ...
                    shape="rounded"
                    color="primary"
                    count={beritaData._meta.pageCount}
                    page={halaman}
          ...
          <div className="col-lg-3">
            ...
            {/* untuk sementara ini tampilkan berita terbaru dulu sembari nunggu api berita populer */}
            <ol className="list-group list-group-numbered">
              {status === "selesai" ? (
                beritaData.items.slice(0, 5).map((item, i) => (
                  <li
    ...
    const fetchData = async () => {
      try {
        ...
        const kategoriResult = await axios.get(
          `${API_URL}/news/categories/${INS_ID}`,
          { cancelToken: source.token, timeout: 60000 }
        );
  ...
  switch (status) {
    ...
    case "selesai":
      return kategoriData.map((item, i) => (
        <Link
          className="list-group-item list-group-item-action"
          to={`/berita/kategori/${kategoriId[i]}`}
          key={i}
        >
          {item}
        </Link>
